export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === "/livescore/tennis/sofascore-live") {
            try {
                // Hacemos fetch a SofaScore agregando Origin y Referer para saltar su WAF
        ...
        try {
            ...
            for (const ev of evs) {
                try {
                    ...
                    // Parseo estricto de Sets ganados y Juegos del último set 
                    let sets1 = parseInt(ev?.Tr1 || "0", 10) || 0;
                    let sets2 = parseInt(ev?.Tr2 || "0", 10) || 0;
                    let games1 = 0;
                    let games2 = 0;

                    if (ev?.Tr1S1 && ev?.Tr1S2) {
                        // A veces la API manda array de scores por set
                        let currentSet = ev.Tr1S5 ? 5 : ev.Tr1S4 ? 4 : ev.Tr1S3 ? 3 : ev.Tr1S2 ? 2 : 1;
                        games1 = parseInt(ev[`Tr1S${currentSet}`] || "0", 10) || 0;
                        games2 = parseInt(ev[`Tr2S${currentSet}`] || "0", 10) || 0;
                    } else if (ev?.Tr1C1 != null) {
                        // Current Set games
                        games1 = parseInt(ev.Tr1C1 || "0", 10) || 0;
                        games2 = parseInt(ev.Tr2C1 || "0", 10) || 0;
                    }
...
// Busca un evento por Eid en el payload de list-by-date y formatea si es tenis
function findEventByEid(payload, eid) {
    ...
    for (const st of stages) {
        const evs = Array.isArray(st?.Events) ? st.Events : [];
        for (const ev of evs) {
            if (String(ev?.Eid || "") === String(eid)) {
                // Si es tenis (viendo Cnm o el payload exterior), añadir las props planas
                const cnm = String(st?.Cnm || "").toUpperCase();
                let isTennis = cnm.includes("ATP") || cnm.includes("WTA") || cnm.includes("CHALLENGER") || cnm.includes("ITF");
                if (payload.category === "tennis") isTennis = true;

                let out = { ...ev };
                if (isTennis || true) { // Añadir siempre que sea posible, index2 usará lo que necesite
                    out.status = epsToStatus(ev.Eps);
                    out.sets1 = parseInt(ev.Tr1 || "0", 10) || 0;
                    out.sets2 = parseInt(ev.Tr2 || "0", 10) || 0;
                    out.games1 = 0;
                    out.games2 = 0;

                    if (ev.Tr1S1 && ev.Tr1S2) {
                        let currentSet = ev.Tr1S5 ? 5 : ev.Tr1S4 ? 4 : ev.Tr1S3 ? 3 : ev.Tr1S2 ? 2 : 1;
                        out.games1 = parseInt(ev[`Tr1S${currentSet}`] || "0", 10) || 0;
                        out.games2 = parseInt(ev[`Tr2S${currentSet}`] || "0", 10) || 0;
                    } else if (ev?.Tr1C1 != null) {
                        out.games1 = parseInt(ev.Tr1C1 || "0", 10) || 0;
                        out.games2 = parseInt(ev.Tr2C1 || "0", 10) || 0;
                    }
                }
                return out;
            }
